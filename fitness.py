...
def _mutual_information_2d(y, y_pred, w):
    """
    ...
    jh = np.histogram2d(y, y_pred, bins=bins)[0]
...
def _mutual_information_sk(y, y_pred, w):
    
    mi = mutual_info_classif(y_pred.reshape(-1, 1), y)
    return mi[0]



mutual_information_sk = _Fitness(function=_mutual_information_sk,
                              greater_is_better=True)

mutual_information = _Fitness(function=_mutual_information_2d,
                              greater_is_better=True)
...
_fitness_map = {'mutual information sk': mutual_information_sk,
