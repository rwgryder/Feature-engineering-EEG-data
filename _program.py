...
class _Program(object):
    ...
    def fitness(self, parsimony_coefficient=None):
        ...
        for node in range(len(self.program)):
            if self.program[node] not in ("add", "sub"):
                vc_len += 1
        penalty = parsimony_coefficient * vc_len * self.metric.sign
        # original penalty    
    ...
    def point_mutation(self, random_state):
        ...
        mutate = range(len(program))
        fitness_curr = self.raw_fitness_
        program_orig = copy(program)
    
        for node in mutate:
            program_cand = copy(program_orig)
            if isinstance(program_cand[node], _Function):
                arity = program_cand[node].arity
                # Find a valid replacement with same arity
                replacement = len(self.arities[arity])
                replacement = random_state.randint(replacement)
                replacement = self.arities[arity][replacement]
                program_cand[node] = replacement
            else:
                    # We've got a terminal, add a const or variable
                if self.const_range is not None:
                    terminal = random_state.randint(self.n_features + 1)
                ...
                if terminal == self.n_features:
                    ...
                    if self.const_range is None:
                        ...
                        raise ValueError('A constant was produced with '
                                             'const_range=None.')
                program_cand[node] = terminal
            self.program = copy(program_cand)
            fitness_cand = self.raw_fitness_
            if self.metric.sign > 0:
                if fitness_cand > fitness_curr:
                    fitness_curr = fitness_cand
                    program = copy(program_cand)
            else:
                if fitness_cand < fitness_curr:
                    fitness_curr = fitness_cand
                    program = copy(program_cand)
